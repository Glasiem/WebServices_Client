...
public class DBMS {
    ...
    public static void main(String[] args) {
        ...
        dbms.rowMenu.add(dbms.deleteRowDuplicates);
                ...
                if (newColumnName != null && !newColumnName.isEmpty()) {
                    ...
                    if (selectedDataType != null) {
                        if (selectedDataType == ColumnType.MONEY_INVL) {
                            ...
                            DoubleInput.InputResult inputResult = dialog.showInputDialog();
                            if (inputResult != null) {
                                String min = inputResult.getMin();
                                String max = inputResult.getMax();
                                System.out.println("Min " + min);
                                System.out.println("Max " + max);
                                result = webServiceSoap.addColumn(selectedTab, newColumnName + "(" +min + "-" + max + ")", org.example.generated.ColumnType.fromValue(selectedDataType.name()),min, max);
                                if (result){
                                    addColumn(selectedTab, newColumnName + "(" +min + "-" + max + ")", selectedDataType.name());
                                } else {
        ...
        dbms.deleteRowDuplicates.addActionListener(e -> {
            int selectedTab = dbms.tabbedPane.getSelectedIndex();
            if (selectedTab != -1) {
                JPanel tablePanel = (JPanel) dbms.tabbedPane.getComponentAt(selectedTab);
                JScrollPane scrollPane = (JScrollPane) tablePanel.getComponent(0);
    ...
    public static java.util.List<Row> getRows(int tableIndex){
        ObjectMapper mapper = new ObjectMapper();
        String result = webServiceSoap.getRows(tableIndex);
        java.util.List<Row> rows = new ArrayList<>();
        try {
            rows = mapper.readValue(result, new TypeReference<java.util.List<Row>>(){});
    ...
    public static java.util.List<Column> getColumn(int tableIndex){
        ObjectMapper mapper = new ObjectMapper();
        ...
        try {
            columns = mapper.readValue(result, new TypeReference<java.util.List<Column>>(){});
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
        return columns;
    ...
    public static java.util.List<TableData> getTableData(){
        ObjectMapper mapper = new ObjectMapper();
        String result = webServiceSoap.getTablesData();
        java.util.List<TableData> tableData = new ArrayList<>();
        try {
